...
%of space, and their amplitude can be further scaled as to realise a
...
%
%AntArray Properties
%   M               matrix containing element's excitations [A]
%   freq            frequency of operation [Hz]
%   norm_freq       frequency used for power normalization [Hz]
%   el_len          dipole's length [mm]
%   spacing         inter-elements spacing [fraction of wavelength]
...
classdef AntArray
    properties (Constant, Access='public')
        ...
        def_min_E = -1.3;   % Minimal electric field for reception [dB V/m]
    end
    properties (GetAccess='public', SetAccess='private')
        ...
        min_E;          % minimal electric field for reception [dB V/m]
        name;           % name used for the figures
        min_XY;         % min dB scale value for XY-patterns
        max_XY;         % max dB scale value for XY-patterns
        min_YZ;         % min dB scale value for YZ-patterns
        max_YZ;         % max dB scale value for YZ-patterns
        min_E_str;      % min dB scale value for E-strength patterns
        max_E_str;      % max dB scale value for E-strength patterns
        dire;           % matrix of element's groupings
        dire_str;       % cell array of element's groupings
        comments;       % comment string, will be printed on elements' plot
        pwr;            % input power [W]
    ...
    methods
        ...
        function obj = AntArray(M, f, l, s, cfg, prefs)
            %ANTARRAY constructor
            %
            % obj = ANTARRAY(M, F, L, S, cfg, prefs)
            % obj = ANTARRAY(path, F, L, S, cfg, prefs)
            % 
            % If cfg and prefs are empty or not specified, a GUI will be
            % displayed for setting up the array parameters
            %
            % INPUT
            ...
            elseif ~isempty(cfg)
                ...
                if prefs && prefs_out ~= -1
                    ...
                    for i=1:length(prefs_fl)
                        for j=1:length(cfg_fl)
                            if strcmp(prefs_fl{i}, cfg_fl{j})
                                cfg_out.(cfg_fl{j}) = prefs_out.(prefs_fl{i});
                            end;
                        end;
                    end;
                end;
                
                ant_fl = fieldnames(AntArray);
                for i=1:length(ant_fl)
                    for j=1:length(cfg_fl)
                        if strcmp(ant_fl{i}, cfg_fl{j})
                            obj.(ant_fl{i}) = cfg_out.(cfg_fl{j});
                        end;
                    end;
                end;
            elseif prefs
                if prefs_out == -1
                    warning 'No preference previously defined';
                else
                    prefs_fl = fieldnames(prefs_out);
                    ant_fl = fieldnames(AntArray);
                    for i=1:length(ant_fl)
                        for j=1:length(prefs_fl)
                            if strcmp(ant_fl{i}, prefs_fl{j})
                                obj.(ant_fl{i}) = prefs_out.(prefs_fl{j});
                            end;
                        end;
                    end;
                end;
        ...
        function [obj, ptrn] = genPattern(obj, d, L, mode, ss, theta)
            ...
            % pattern is the region where the electric field strength is
        ...
        function obj = E_strength(obj, L, y, z, smp)
            ...
            %   y:      (optional) Y-position of the line [mm]
            %   z:      (optional) Z-position of the line [mm]
        ...
        function plotWeight(obj, mode, d)
            ...
            for ii=1:length(spe_ticks_x)
                spe_ticks_x(ii) = range_x(1) + ...
                    (ii-1)*2*x_dev(end)*fact_x/tick_fact_x;
            end;
            spe_ticks_pos_x = spe_ticks_x+ss_x*fact_x/2;
            spe_ticks_y = zeros(tick_fact_y+1,1);
            for ii=1:length(spe_ticks_y)
                spe_ticks_y(ii) = range_y(1) + ...
                    (ii-1)*2*y_dev(end)*fact_y/tick_fact_y;
            end;
        ...
        function saveCfg(obj)
            ...
            sav_loc = [obj.datetime(1:8) '/cfg'];
            sav_name = obj.datetime(9:end);
    
            if ~isempty(obj.name)
                if exist([sav_loc '/' sav_name '.cfg'], 'file')
            ...
            save([sav_loc '/' sav_name '.cfg'], 'structarr', ...
                '-MAT', '-v7.3');
        end
    ...
    methods (Access='private')
        ...
        function ptrn = E_YZ(obj, d, L, ss)
            ...
            % OUTPUT
            %   ptrn:   2D matrix of electric field strengths [dB V/m]
        ...
        function ptrn = E_YZ_main(obj, d, L, ss)
            ...
            plotdata = plotdata.*(obj.min_E-40); % 40dB below threshold
        ...
        function ptrn = E_XY(obj, d, L, ss)
            ...
            %   ss:     Step size for the plot [m]
            % OUTPUT
            %   ptrn:   2D matrix of electric field strength [dB V/m]
        ...
        function [E_x, E_y, E_z] = E_array(obj, x, y, z)
            ...
            % OUTPUT
            %   E_{k}:  Electric field components in the {k}-direction [V/m]
        ...
        %% Function to compute denominator of directivity expression
        function E_strength = dir_den(obj, r, theta, phi)
            %dir_den computes the integrand for the denominator of the
            %standard expression for directivity
            %
            % INPUT
            %   obj:    AntArray object
            %   r:      distance to the array centre [m]
            %   theta:  matrix of theta angles [rad]
            %   phi:    matrix of phi angles [rad]
            % OUTPUT
            %   E_strength: matrix of denominator values [Vm]
        ...
        %% Function to compute directivity numerator
        function E_strength = E_tot(obj, r, theta, phi)
            ...
            %   phi:    phi angle [rad]
            % OUTPUT
            %   E_strength: matrix containing square of electric field
            %               strength components
        ...
        function obj = normalize(obj)
            %normalize rescale the input currents in order to achieve the
            %desired input power
            %
            % INPUT
            %   obj:    AntArray object
            % OUTPUT
            %   obj:    AntArray object with rescaled currents
    ...
    methods (Static, Access = 'private')
        ...
        function [E_x, E_y, E_z] = E_dipole(l, I, f, c1, c2, c3, spheric)
            % Compute the electric field components generated by a dipole
            % antenna at a certain position in space
            %
            % The position can be expressed in cartesian or spherical coordinates:
            %   cartesian:  [c1, c2, c3] = [x, y, z]
            %   spherical:  [c1, c2, c3] = [r, theta, phi]
            %
            % INPUT
            %   l:          length of the dipole [m]
            %   I:          constant current through the dipole [A]
            %   f:          frequency of operation [Hz]
            %   c1,c2,c3:   position wrt the dipole centre
            %   spheric:    (optional) c's are spherical coordinates? [default = 0]
        ...
        function filename = openFile(savname)
            % INPUT
            %   savname:    name of AntArray object
            % OUTPUT
            %   filename:   path to the saved AntArray object
        ...
        function res = loadCfg(infile)
            % INPUT
            %   infile:     path to cfg-file location
            % OUTPUT
            %   res:        content of the cfg-file
        ...
        function res = loadPrefs()
            % OUTPUT
            %   res:    structure containing the preferences
        ...
        function names = paramNames()
            % OUTPUT
            %   names:  cell array containing AntArray parameter names
